...
readParamsFromTable(rParticle=0.5, rRelFuzz=0,rCoff=2,top_limit=60,bot_limit=1,width=10,tot_limit=10,toy_limit=10,ejex_limit=150,num_spheres=2000,thick = 0.01,key='_define_a_name_',stabilityThreshold=0.1, maxLoad= 1.5e8, minLoad= 1e1)##Ojo,que el primer while del unbalanced force nunca llego al valor 0.01 del stabilityThreshold
...
#Material paredes
O.materials.append(FrictMat(young=5e9,poisson=0.5,frictionAngle=0,density=3000,label='walls'))
## create walls around the packing
##walls=aabbWalls([mn,mx],thickness=thick,material='walls')
##wallIds=O.bodies.append(walls)
#Define Materials
Rockfill=O.materials.append(FrictMat(young=25e9,poisson=0.3,frictionAngle=radians(30),density=2500,label='spheres'))

surf = gts.read(open('talud3.coarse.gts')) #gts talud estable con escala aumentada X=150, Y=60, Z=10
surf2 = gts.read(open('Gta_paralelepipedo.gts')) #paredes gts forma paralelepipedo, impide que las particulas caigan fuera
# Muestra la geometria gts en el modelo
gtstalud=O.bodies.append(pack.gtsSurface2Facets(surf2, color=(1, 0, 1)))
